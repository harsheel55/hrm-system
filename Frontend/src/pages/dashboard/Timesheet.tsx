        ...
        <div className="min-h-screen bg-[#f8fafc] pb-12">
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Time Tracking</h1>
                        <p className="text-slate-500 text-sm font-medium mt-1">Log your project hours and submit for manager approval.</p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button className="flex items-center px-6 py-2.5 bg-indigo-600 text-white rounded-2xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 text-sm font-bold">
                    ...
                    {timesheetStats.map((stat, i) => (
                        <div key={i} className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all group cursor-pointer">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-1 rounded-md">{stat.change}</span>
                            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                            <div className="p-8 border-b border-slate-50 flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button className="flex items-center gap-1 px-4 py-2 bg-slate-50 text-slate-600 rounded-xl text-xs font-bold hover:bg-indigo-50 hover:text-indigo-600 transition-all">
                                        ...
                                        {timesheetData.map((project, i) => (
                                            <tr key={i} className="hover:bg-slate-50/80 transition-colors group">
                                                <td className="px-6 py-5 text-left">
                                                    <p className="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{project.project}</p>
                                                    <p className="text-[10px] font-medium text-slate-400 uppercase">Development</p>
                                                </td>
                                                {[project.mon, project.tue, project.wed, project.thu, project.fri, project.sat, project.sun].map((hr, idx) => (
                                                    <td key={idx} className={`px-2 py-5 font-mono text-sm ${hr === 0 ? 'text-slate-200' : 'text-slate-600 font-bold'}`}>
                                                        {hr}
                                                    </td>
                                                ))}
                                                <td className="px-4 py-5 font-mono font-black text-slate-900 bg-slate-50/30">
                                                    {project.total}
                                                </td>
                        ...
                        {/* Submission Warning */}
                        <div className="bg-amber-50 rounded-[2rem] border border-amber-100 p-6 flex gap-4 items-start">
                            <div className="p-2 bg-white rounded-xl shadow-sm text-amber-600">
                                <AlertCircle size={20} />
                            </div>
                            <div>
                                <h4 className="text-sm font-bold text-amber-900">Review Required</h4>
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
                            ...
                            <div className="flex items-end justify-between gap-2 h-32 mb-8 px-2">
                                {weeklySummary.map((day, i) => (
                                    <div key={i} className="flex flex-col items-center gap-2 w-full group">
                                        <div 
                                            className={`w-full rounded-t-lg transition-all duration-500 ${day.percentage > 0 ? 'bg-indigo-500 group-hover:bg-indigo-600' : 'bg-slate-100'}`} 
                            ...
                            <div className="space-y-4">
                                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex justify-between items-center">
                                    <p className="text-xs font-bold text-slate-500 uppercase">Avg. Productive Time</p>
                                    <p className="text-lg font-black text-slate-900">8.4h</p>
                                </div>
                                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex justify-between items-center">
                                    <p className="text-xs font-bold text-slate-500 uppercase">Project Overlap</p>
                                    <p className="text-lg font-black text-slate-900">03 Tasks</p>
                                </div>
                            </div>
                        </div>
